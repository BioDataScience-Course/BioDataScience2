...
# CAH for SciViews, version 1.0.0
...
# We also use metric = instead of method = to avoid confusion with method
# argument of hclust()/cluster() and to be compatible with cluster::daisy()
...
# Depending on metric =, use either vegan::vegdist or stats::dist as default fun
dissimilarity <- function(data, formula = ~ ., subset = NULL,
metric = "euclidean", scale = FALSE, rownames = "rowname", transpose = FALSE,
fun = NULL, ...) {
  # TODO: get more meaningful warn ings and errors by replacing fun by actual
  # name of the function
  if (is.null(fun)) {
    if (metric %in% c("maximum", "binary", "minkowski")) {
      fun <- stats::dist
    } else {
  ...
  if (!is.null(as.list(args(fun))$metric)) {# metric = instead of method =
    dst <- fun(data, metric = metric, stand = scale, ...)
  } else if (isTRUE(scale)) {
    if (is.null(as.list(args(fun))$stand)) {# fun has no stand = argument
      data <- scale(data)
      dst <- fun(data, method = metric, ...)
    } else {# We don't do it ourself because there may by also qualitative or
      # binary data (like for cluster::daisy, for instance)
      dst <- fun(data, method = metric, stand = scale, ...)
    }
  } else {# Just method = and scale = FALSE
    dst <- fun(data, method = metric, ...)
  }
  attr(dst, "call") <- match.call()
  # Depending if it is a dist or dissimilarity object, the metric is stored in
  # method or in Metric, but we use metric
...
as.dissimilarity.matrix <- function(x, ...) {
  dst <- as.dist(x, ...)
  attr(dst, "call") <- match.call()
  attr(dst, "Metric") <- attr(dst, "method")
  class(dst) <- unique(c("dissimilarity", class(dst)))
  dst
}

# We want to print only the first few row and columns
print.dissimilarity <- function(x, digits.d = 3L, rownames = "labels", ...) {
  mat <- as.matrix(x)
  ...
  # TODO: replace this by tibble::add_column
  tbl <- dplyr::bind_cols(
    as_tibble_col(rownames(mat), column_name = rownames), tbl)
  tbl <- tbl[, -ncol(tbl)]
  more_info <- ""
  ...
  cat("Dissimilarity matrix with metric: ", attr(x, "method"),
    more_info, "\n", sep = "")
  print(tbl)
  invisible(x)
}
...
cluster.dist <- function(x, method = "complete", fun = NULL, ...) {
  ...
  # If the orignial data were scaled or transposed, get the info also
  clst$rownames <- attr(x, "rownames")
...
# predefined methods... and here, it really something that predict() is made for
...
augment.cluster <- function(x, data, k = NULL, h = NULL, ...) {
  ...
  if (isTRUE(x$transpose)) {
    ...
    if (!is.matrix(data) && !is.null(data[[rownames]])) {
      rownames(data) <- data[[rownames]]
      data[[rownames]] <- NULL
    }
    data <- t(data)
...
# circular), see http://www.sthda.com/english/wiki/beautiful-dendrogram-visualizations-in-r-5-must-known-methods-unsupervised-machine-learning
# TODO: how can I get a scale for height with the circular type?
...
lab = "Height", ...) {
  ...
  if (type == "circular") {
    if (!missing(hang))
      warning("'hang' is not used with a circualr dendrogram")
    ...
    if (type == "horizontal") {
      ...
      plot(dendro, horiz = FALSE, ylab = lab, ...)
    }
  }
}
...
- Vous préparer à analyser de manière autonome un jeu de données multivariées à l'aide de la CAH
...
A partir du jeu de données `fish`, calculez la matrice de distances entre stations en utilisant l'indice de Bray-Curtis et `dissimilarity()`, considérant que toutes les fonctions nécessaires sont déjà chargées dans R. Pour une aide concernant les arguments de la fonction, voyez `?vegan::vegdist` en substituant `metric =` à la place de `method =`.
...
print(fish_dist <- dissimilarity(___, metric = "___"))

#### ATTENTION: Hint suivant = solution !####
```

```{r fish_bray_h2-solution}
print(fish_dist <- dissimilarity(fish, metric = "bray"))
```

```{r fish_bray_h2-check}
grade_code("Vous venez de réaliser votre première matrice de distances. L'argument `metric =` de la fonction `dissimilarity()` précise la métrique utilisée : \"bray\" pour Bray-Curtis, \"canberra\" pour l'indice de Canberra, \"euclidean\" pour la distance Euclidienne et \"manhattan\" pour la distance de Manhattan, ...")
```
...
A partir des données contenues dans `envir`, calculez la matrice de dissimilarités en utilisant la distance Euclidienne et *sans* transformer les données. Pour rappel, les métriques sont définies comme l'argument `method = ` de `?vegan::vegdist`.
...
print(envir_dist <- ___(___, metric = "___"))

#### ATTENTION: Hint suivant = solution !####
```

```{r euclidean1_h2-solution}
print(envir_dist <- dissimilarity(envir, metric = "euclidean"))
```
...
Calculez une nouvelle matrice de dissimilarités de distances euclidiennes, mais cette fois en standardisant les données. 
...
print(envir_dist <- dissimilarity(___, metric = "___", ___))
```

```{r euclidean2_h3-hint-2}
print(envir_dist <- dissimilarity(___, metric = "euclidean", ___ = TRUE))

#### ATTENTION: Hint suivant = solution !####
```

```{r euclidean2_h3-solution}
print(envir_dist <- dissimilarity(envir, metric = "euclidean", scale = TRUE))
```
...
```{r hac_prep}
envir_dist <- dissimilarity(envir, metric = "euclidean", scale = TRUE)
```
...
Notez que vous repérez le niveau de coupure avec le même code, quelle que soit la variante du dendrogramme utilisée.

## Interprétation

Une fois le dendrogramme réalisé et le niveau de coupure choisi, nous devons récupérer les groupes ainsi constitués afind de les interpréter. Vous utilisez `predict()` pour récupérer un vecteur nommé (nom des stations de 1 à 30 au dessus, groupes de 1 à 4 en dessous)\ :
...
```{r, echo=TRUE}
fish_dist <- dissimilarity(fish, metric = "bray")
...
Ce genre de résultat est assez caractéristique de données biologiques\ : la concordance n'est pas parfaite, mais une belle tendance se dégage avec les correspondances qui peuvent être résumées comme suit\ :
...
envir_dist <- dissimilarity(envir, metric = "euclidean",
