...
Réalisez la **régression linéaire polynomiale d'ordre 2** de la variable `y` en fonction de la variable `x` sur le jeu de données `df` et représentez-la graphiquement. Vous avez à votre disposition un nuage de points et un résumé des données pour en connaissance.
...
Réalisez une régression **linéaire simple** et une régression **linéaire polynomiale d'ordre 2** de la variable `y` en fonction de la variable `x` dans le jeu de données `df`. Utilisez le critère d'Akaike afin de déterminer quel est le meilleur modèle. Vous avez à votre disposition un nuage de points et un résumé des données pour en prendre connaissance.
...
# régression linéaire simple (snippet 1)
summary(lm. <- lm(data = DF, YNUM ~ XNUM))
# régression linéaire polynomiale (snippet 2)
summary(lm. <- lm(data = DF, YNUM ~  XNUM + I(XNUM^2)))
# comparaison des 2 modèles
___(lm_lin_simp, lm_lin_poly)
