...
urchin <- read(___, package = ___, lang = "FR")
...
```{r}
read("ToothGrowth", package = "datasets") %>.%
  fmutate(., dose = as.factor(dose)) %>.%
  labelise(.,
    label = list(len  = "Longueur des dents", supp = "Supplémentation", dose = "Dose"),
    units = list(len  = "mm")) ->
  tooth_growth
...
read("ToothGrowth", package = "datasets") %>.%
  fmutate(., dose = as.factor(dose)) %>.%
  labelise(.,
    label = list(len  = ___, supp = __, dose = "Dose"),
    units = list(len  = "mm")) ->
  tooth_growth
...
read("ToothGrowth", package = "datasets") %>.%
  fmutate(., dose = as.factor(dose)) %>.%
  labelise(.,
    label = list(len  = "Longueur des dents", supp = ___, dose = "Dose"),
    units = list(len  = "mm")) ->
  tooth_growth
...
$$bmi = mass\ [kg]/height [m]^2$$ Vous travaillez toujours sur le jeu des données `biometry` du package {BioDataScience} qui comprend, pour rappel, les variables suivantes : `r names(biometry)`.

```{r qu_mutate_h2, exercise=TRUE, exercise.lines=10}
biometry <- read("biometry", package = "BioDataScience", lang = "FR")
# Calcul du bmi
biometry <- fmutate(___, height_m = ___/100, bmi = ___/___^2)
...
biometry <- read("biometry", package = "BioDataScience", lang = "FR")
# Calcul du bmi
biometry <- fmutate(biometry, height_m = ___/100, bmi = ___/height_m^2)
# Résumé de jeu de données
summary(biometry)
...
biometry <- read("biometry", package = "BioDataScience", lang = "FR")
# Calcul du bmi
biometry <- fmutate(biometry, height_m = height/100, bmi = weight/height_m^2)
# Résumé de jeu de données
summary(biometry)
```

```{r qu_mutate_h2-check}
grade_code("Vous êtes capable d'utiliser la fonction fmutate().")
```

## Rendement de patates

Des agronomes belges s'intéressent aux rendements de deux variétés de pomme de terre : la bintje et la fontane. Après 110 jours, ils réalisent des prélèvements sur différentes parcelles en Belgique dont ils quantifient la production, exprimée en tonnes par hectare.

```{r, warning=FALSE, message=FALSE}
set.seed(42)
pdt <- dtx(
  rendement = c(rnorm(18L, mean = 33.6, sd = 4), rnorm(28L, mean = 39.5, sd = 4)), 
  variete   = c(rep("bintje", times = 18L), rep("fontane", times = 28L))
)
```

Reproduisez le tableau ci-dessous, le jeu de données mis à votre disposition se nomme `pdt` et les variables sont les suivantes : `r names(pdt)`. La fonction de mise en forme du tableau est `knitr::kable()`.
...
L'association wallonne de l'élevage dispose de plusieurs centres d'insémination. Ils ont un recensement des différents taureaux reproducteurs. Réalisez par vous-même le graphique ci-dessous. Notez bien que ce dernier ne montre que les individus âgés de 20 à 55 mois. Vous avez à votre disposition le jeu de données `bull` qui comprend les variables suivantes : `r names(bull)`.

```{r}
bull %>.%
  fsubset(., age >= 20 & age <= 55) %>.%
...
# Le jeu de données bull est déjà chargé en mémoire
...
# La fonction pour filtrer des lignes d'un tableau selon une ou plusieurs conditions est fsubset()
```
...
# Les conditions au sein de la fonction fsubset() s'écrivent avec des opérateurs logique >, >=, <=, <, == ou !=
...
set.seed(43)
weight <- dtx(
  weight = c(rnorm(15L, mean = 100, sd = 5), rnorm(15L, mean = 102, sd = 5)), 
  area   = factor(rep(c("a", "b"), each = 15L))
)
```
