...
-   Découvrir quelques notions liées à la SciViews Box 2022
...
La matière est bien trop vaste pour revoir tous les concepts du premier cours en un seul tutoriel (il est déjà très conséquent). Il se veut une synthèse des éléments principaux afin de vous donner une idée de votre niveau aujourd'hui. N'hésitez pas à réviser les parties du cours 1, si vous en ressentez le besoin.

## Logiciels

Commencez par vous assurer que vous maîtrisez les outils logiciels que nous allons employer dans le cadre de ce cours. Si vous avez des doutes à ce sujet, consultez le premier module de cours de [science des données 1](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2022/d%25C3%25A9couverte-des-outils.html). Répondez aux questions ci-dessous.
...
La [section 5.1 du cours de SDD I](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2022/importation-des-donn%25C3%25A9es.html) traite de l'importation des données dans R. Consultez cette dernière au besoin.

## Visualisation des données

Les graphiques dans R peuvent être de différents type : des graphiques R de base, {lattice} ou {ggplot2}. Une comparaison des moteurs graphiques est réalisée dans la [section 4.5 du cours de SDD I](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2022/diff%25C3%25A9rents-moteurs-graphiques.html). Dans SciViews-R, nous vous proposons d'employer la fonction `chart()` qui permet de réaliser un graphique dans chacun de ces trois moteurs. Cependant, par défaut, il s'agit d'un graphique {ggplot2} auquel nous ajoutons une série de couches avec l'opérateur `+`.
...
Il n'est pas aisé de se souvenir du nom de chaque fonction que l'on peut employer avec `chart()` ou `ggplot()`. Nous vous proposons pour ce faire une syntaxe alternative. La fonction `gg$` associée au pipe natif de R `|>` permet d'obtenir une liste de toutes les fonctions disponibles que l'on peut ajouter au graphique. Le pipe `|>` va remplacer le `+` que l'on suit par `gg$` et une liste de complétion apparaît. Il s'agit des fonctions habituelles comme `geom_point()` par exemple.
...
Le tableau de données se nomme `potatoes`. Ce tableau est constitué des variables `r names(potatoes)`. Les premières lignes du tableau sont affichées ci-dessous.
