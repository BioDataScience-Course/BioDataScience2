...
Le package {tabularise} est ajouté à la SciViews Box 2023. Ce package a une fonction centrale que vous devez retenir `tabularise()`. Cette fonction permet d'obtenir des tableaux bien formatés pouvant être ajoutés à vos carnets de notes et vos rapports. Cette fonction utilise la structure suivante : tabularise\$xxx(objet). En fonction du type de l'objet, la fonction va proposer un tableau spécifique à l'objet. La section`xxx` permet de définir un type particulier. Dans l'exemple ci-dessus, vous avez employé le type `headtail` qui permet d'afficher le début et la fin de votre tableau de données.
...
Vous aurez remarqué que le dossier docs/ n'est pas présent dans le projet proposé ci-dessus. En effet, les rapports et les carnets de notes se trouveront à la racine du projet dorénavant. Cela va vous permettre de définir plus facilement le chemin d'accès relatif de vos fichiers.

Jusqu'à présent vous avez rédigé vos rapports et vos carnets de notes en R Markdown (par exemple : sharks.Rmd). Cette année, nous changeons de format de fichiers pour vos rapports. Nous passons au format Quarto (par exemple : sharks.qmd). Il s'agit d'une nouvelle version du R Markdown proposé par la société Posit (anciennement RStudio). En pratique cela ne change rien pour vous, tout comme le R Markdown votre document va comprendre un entête YAML, des zones de texte rédigé en mardown et du code à l'intérieur de chunk.
...
Le tableau de données se nomme `potatoes`. Ce tableau est constitué des variables `r names(potatoes)`. Les premières et dernières lignes du tableau sont affichées ci-dessous **en utilisant notre nouvelle fonction `tabularise()` bien entendu.**
...
Nous introduisons une troisième série de fonctions : les fonctions fast. Ces fonctions réalisent un résumé d'un échantillon (moyenne, écart type, médiane ...). Dans R de base, nous avons `mean()`, `sd()` ou `median()`. Et bien nous vous présentons leurs fonctions jumelles (enfin, pas complètement, elles sont en fait à la fois bien plus rapides et ont encore d'autres qualités). Elles sont repérables à leur préfixe "f". Ainsi, `mean()` devient `fmean()`, `sd()` devient `fsd()` et ainsi de suite.

Sachant tout cela, voyons un exemple de traitement classique pour résumer un tableau de données, vous utilisiez :

```{r, echo = TRUE, eval=FALSE}
potatoes %>.%
  sgroup_by(., cultivar) %>.%
  ssummarise(., 
    yield_mean = fmean(yield), yield_sd = fsd(yield), n = fn(yield)) ->
  potatoes_red
```
...
grade_code("Fantastique ! Vous avez bien en tête la syntaxe. Pour améliorer visuellement ce tableau à l'impression, on aurait pu utiliser la fonction knitr::kable() ou encore tabularise()", "Il semble que vous avez commis une erreur. Nous vous conseillons de revoir le module 5 de SDD I.")
```
