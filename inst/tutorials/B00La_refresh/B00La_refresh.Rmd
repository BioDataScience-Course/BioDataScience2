...
Ce cours de [Science des données II : analyse et modélisation](https://wp.sciviews.org/sdd-umons2/) fait suite au cours de [Science des données I : visualisation et inférence](https://wp.sciviews.org/sdd-umons/). Par conséquent, il est important que vous ayez bien assimilé la matière du premier cours. Ce tutoriel vise à :

-   Réviser les notions essentielles du cours de Science des données I
-   Découvrir quelques notions liées à la SciViews Box 2023
...
La fonction`SciViews::R()` est employée afin de charger une série de packages qui forme le "dialecte" SciViews-R. Nous l'utilisons en début de script R ou encore dans le premier chunk d'un rapport d'analyse au format R Markdown.
...
Nouveauté dans la SciViews Box 2023 : des packages supplémentaires peuvent être chargés grâce à `SciViews::R()`. Ils correspondent à des sections complémentaires qui ne sont pas chargées de base, par exemple, `"model"` pour la modélisation. Il est également possible à présent de définir le langage par défaut pour l'importation de données qui supportent plusieurs langages et pour les labels automatiques des axes des graphiques via l'argument `lang=`.

Vous remarquerez que l'instruction `SciViews::R` ne génère plus de sortie à présent. Avant, vous aveiz un long listing des packages changés et ensuite des conflits entre fonctions. Ceci est rendu silencieuxn mais vous pouvez toujours préciser l'argument `silent = FALSE`, donc avec `SciViews::R(silent = FALSE)` pour retrouver le rapport auquel vous être habitué.
...
Importez le tableau nommé `crabs` qui provient du package {MASS}. **rappel : lorsqu'un exercice comporte du code avec des zones à remplacer (`___`), ne faites rien d'autre que complétez ces zones. Les outils d'autocorrection sont très sensibles et n'apprécieraient pas autrement.**
...
```{r qu_read}
quiz(
  question("Depuis le fichier sharks_report.Rmd, Sélectionnez l'instruction R correcte afin de lire le tableau `sharks.csv`.",
  ...
  question("Depuis le fichier sharks_import.R, Sélectionnez l'instruction R correcte afin de lire le tableau `sharks.csv`.",
    answer("sharks <- read(\"../data/sharks.csv\")"),
    answer("sharks <- read(\"sharks.csv\")"),
    answer("sharks <- read(\"data/sharks.csv\")", correct = TRUE),
    answer("sharks <- read(\"../../data/sharks.csv\")"),
    ...
    allow_retry = TRUE, random_answer_order = TRUE,
    submit_button = "Soumettre une réponse", try_again_button = "Resoumettre une réponse",
    correct = "Très bien. Le script R utilise comme point de départ le dossier de base du projet RStudio, contrairement au fichier Rmd qui part du dossier qui le contient.",
  incorrect = "Ce n'est pas juste. Révisez les chemins relatifs dans le projets RStudio."
    )
  )
```

La [section 5.1 du cours de SDD I](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2023/importation-des-donn%25C3%25A9es.html) traite de l'importation des données dans R. Consultez cette dernière au besoin.

![](images/breaking_news.jpg){width="30%"}

-   Petit changement dans les types de fichiers : les bloc-notes et les rapports seront rédigés sous forme de documents **[Quarto](https://quarto.org)** (certains peuvent encore être au format R Markdown, toutefois). L'extension de fichier est `.qmd` (contre `.Rmd` pour R Markdown). Il y a relativement peu de changements dans l'utilisation de ce format par rapport à R Markdown, à part dans l'entête YAML (mais qui sera en grande partie pré-rédigée pour vous dans les projets). Par contre, un éditeur visuel et des fonctionnalités supplémentaires de Quarto vous faciliteront la vie... Vous les découvrirez au fur et à mesure.

-   Autre changement concernant l'organisation des dossiers et des fichiers dans les projets : vous remarquerez que les bloc-notes et les rapports ne seront plus dans le sous-dossier `/docs` dans vos projets, mais directement à la racine. Deux raisons à cela : étant les documents clés du projet, ils sont plus facilement gérés depuis sa racine, et seconde raison, le dossier courant du document Quarto est ainsi le même que le dossier courant du projet (à la console R). C'est plus facile à gérer !

-   Et tant que nous évoquons la rdaction de rapport, de nouveaux outils font leur apparition pour obtenir des beaux tableaux (fonction `tabularise()`) et des belles équations (fonctions `eq_()` et `eq__()`). Voici par exemple un tableau de l'analyse de variance nouvelle mouture :
...
Les graphiques dans R peuvent être de différents types : des graphiques R de base, {lattice} ou {ggplot2}. Une comparaison des moteurs graphiques est réalisée dans la [section 4.5 du cours de SDD I](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2023/diff%25C3%25A9rents-moteurs-graphiques.html). Dans SciViews-R, nous vous proposons d'employer la fonction `chart()` qui permet de réaliser un graphique dans chacun de ces trois moteurs. Cependant, par défaut, il s'agit d'un graphique {ggplot2} auquel nous ajoutons une série de couches avec l'opérateur `+`.
...
Il n'est pas aisé de se souvenir du nom de chaque fonction que l'on peut employer avec `chart()` ou `ggplot()`. Nous vous proposons pour ce faire une syntaxe alternative. La fonction `Sgg$` associée au pipe natif de R `|>` permet d'obtenir une liste de toutes les fonctions disponibles que l'on peut ajouter au graphique. Le pipe `|>` va remplacer le `+` que l'on suit par `Sgg$` et une liste de complétion apparaît. Il s'agit des fonctions habituelles comme `geom_point()` par exemple. *Vous en comprendrez surtout l'intérêt lorsque vous utiliserez cette fonction par vous-même dans RStudio, où vous verrez les aides apparaitre.*
...
```{r chart_h4-check}
grade_code("Bien joué ! Vous êtes capable d'employer la formule dans chart() ainsi que le bon geom_* afin d'obtenir des boites de dispersion.")
```
...
Le tableau de données se nomme `potatoes`. Ce tableau est constitué des variables `r names(potatoes)`. Les premières lignes du tableau sont affichées ci-dessous **en utilisant notre nouvelle fonction `tabularise()` bien entendu.**

```{r, echo=TRUE}
tabularise(head(potatoes))
```

Afin de sélectionner les colonnes `yield`, `cultivar` du tableau `potatoes`, nous aurions employé dans les versions antérieures de la SciViews Box :

```{r, echo = TRUE, eval = FALSE}
potatoes1 <- select(potatoes, yield, cultivar)
```
...
select(potatoes,  yield, cultivar) |> collect_dtx() ->
  potatoes1
```

Vous aurez remarqué que nous employons tout d'abord la fonction `sselect()` qui est similaire à `select()`.

Pour fixer les idées, nous pouvons décider de regrouper les fonctions de R en familles selon leur comportement. Les fonctions d'une même famille ont un comportement similaire du point de vue des arguments qu'elles acceptent, du type d'objet qu'elles utilisent et de la façon dans les résultats sont renvoyés.

Le [Tidyverse](https://www.tidyverse.org) propose un ensemble de fonctions cohérentes autour des objets **tibble**s. Ce sont les fonctions comme `mutate()`, `select()`, `filter()`, `group_by()`, `summarise()`, `pivot_longer()`,... que nous utilisions jusqu'ici. Nous applerons cette famille les **fonctions tidy**.

Dans la SciViewsBox 2022, nous vous présentons des fonctions jumelles aux précédentes qui se comportent de manière très proche et qui réalisent le mùeme traitement dans 95% des cas, mais qui sont nettement plus rapides. Pour cette raison, nous appellerons cette famille, les **fonctions speedy**. Leur nom est identique aux fonctions tidy, mais préfixé d'un "s" pour les différencier. Donc, l'homologue de `mutate()` est `smutate()`, `filter()` devient `sfilter()`, et ainsi de suite...
...
Étant donné le changement en interne du type d'objet pour les tableaux de données vers **data.table**, avec les fonctions tidy, il faut prendre soin de **collecter** les résultats à la fin. Dans SciViews-R, on utilisera l'assignation alternative `%<-%` ou `%->%`, ou alors, on utilisera explicitement `collect_dtx()` pour récupérer un data frame utilisable ensuite avec les fonctions non-tidy.

**Autre changement :** le pipe natif `|>` est apparu dans la version 4.1.0 de R. Vous connaissez déjà deux pipes que sont le pipe de {magrittr} `%>%` ou le pipe du package {svFlow} `%>.%`. Ces trois opérateurs permettent de chaîner des instructions afin de montrer de manière claire la suite des opérations. Les pipes améliorent grandement la lisibilité du code. Notez que le pipe `|>` ne requiert pas l'utilisation du `.` dans la fonction qui le suit alors que le pipe `%>.%` l'impose. Vous pouvez retrouver de plus amples informations sur le chaînage des instructions dans la [section 5.5 de SDD I](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2022/cha%25C3%25AEnage-des-instructions.html).

Nous vous proposons de continuer à utiliser le pipe `%>.%` de SciViews-R pour relier des étapes successives de traitement des données en allant à la ligne après l'opérateur pour bien indiquer que c'est une étape suivante. Par contre, s'il s'agit de combiner deux fonctions pour obtenir une seule étape de traitements, nous suggérons de préférer le pipe de base de R `|>` et de placer le tout sur la même ligne. Une situation qui se rencontre suivant est le `group_by()` suivi d'un `summarise()`. La fonction `group_by()` ne modifie en rien le tableau. Elle le prépare pour `summarise()` en indiquant quelle(s) variable(s) vont servir au regroupent, dont `summarise()` se chharge ensuite. Donc, il nous semble opportun d'indiquer que c'est une seule et même étape en écrvant : `group_by(...) |> summarise(...)`.

Voyons tout cela sur des exemples concrets. Sélectionnez les parcelles ayant un rendement strictement supérieur à 30 tonnes par hectare et inférieur ou égal à 41 tonnes par hectare. Utilisez une fonction speedy pour ce faire.
...
Nous introduisons une troisième série de fonctions : les fonctions fast. Ces fonctions réalise un résumé d'un échantillon (moyenne, écart type, médiane, ...). Dans R de base, nous avons `mean()`, `sd()` ou `median()`. Et bien nous vous présentons leurs fonctions jumelles (enfin, pas complètement, elles sont en fait à la fois bien plus rapides et ont encore d'autres qualités). Elle sont repérables à leur préfixe "f". Ainsi, `mean()` devient `fmean()`, `sd()` devient `fsd()` et aisni de suite.

Sachant tout cela, voyons comment un traitement classique est transformé. Auparavant, pour résumer un tableau de données, vous utilisiez :

```{r, echo = TRUE, eval=FALSE}
potatoes %>.%
  group_by(., cultivar) %>.%
  summarise(., 
    yield_mean = mean(yield), yield_sd = sd(yield), n = n()) ->
  potatoes_red
```

![](images/breaking_news.jpg){width="30%"}

En appliquant les nouvelles conventions expliquées plus haut, nous vous proposons d'employer à présent l'une des deux formes suivantes (avec une préférence pour la première).
...
-   des fonctions fast `fmean()`, `fsd()` et `fn()`
-   du pipe natif `|>` et du pipe `%>.%` selon le contexte
-   de l'assignation alternative `%->%` en fin de pipeline

*Nous vous conseillons d'employer préférentiellement les fonctions fast maintenant qui sont plus rapides.*

```{r, echo=TRUE}
list_fstat_functions()
```

C'est à vous de jouer ! Réalisez un tableau résumé reprenant le rendement médian, le rendement minimal, le rendement maximal et le nombre d'observations de parcelles par variété. Utilisez les fonctions speedy et fast. Utilisez de plus le pipe natif de R lorsque c'est judicieux. Les variables de `potatoes` à votre disposition sont : `r names(potatoes)`
...
grade_code("Fantastique ! Vous avez compris la nouvelle syntaxe. Pour améliorer visuellement ce tableau à l'impression, on aurait pu utiliser la fonction knitr::kable()", "Il semble que vous avez commis une erreur. Nous vous conseillons de revoir le module 5 de SDD I.")
```

## Test d'hypothèse

Dans le cours de science des données I, cinq modules ont été consacrés entièrement ou en partie à la réalisation de test d'hypothèse. Le nombre conséquent de modules vous permet de vous rendre compte de l'importance de cette matière. L'exercice suivant traite du test *t* de Student. Ce test permet de comparer les moyennes de deux groupes. Vous allez travailler pour ce faire sur des taureaux reproducteurs.
...
grade_code("Bravo ! Vous savez réalisez un test *t* de Student.")
```
