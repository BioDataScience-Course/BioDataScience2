...
Importez le tableau nommé `crabs` qui provient du package {MASS}. **Note : lorsqu'un exercice comporte du code avec des zones à remplacer (`___`), ne faites rien d'autre que compléter ces zones. Les outils d'autocorrection sont très sensibles et n'apprécieraient pas autrement.**
...
Le tableau de données se nomme `potatoes`. Ce tableau est constitué des variables `r names(potatoes)`. Les premières et dernières lignes du tableau sont affichées ci-dessous.
